...
- **도형 조작** — 드래그로 이동, `Q`/`W` 30° 회전, `E` 뒤집기
- **삼각형 격자** — 점선 격자, 교점·중점 스냅 (설정에서 on/off)
- **도형 스냅** — 다른 조각의 모든 꼭짓점·변 중점에 맞추기
- **개발자 모드** — `/dev-mode.html`에서 도형 개별 삭제(`X`), 배치 JSON 내보내기·불러오기
- **교사 모드** — `/teacher-mode.html`에서 예시 배치(삼각형·직사각형·볼록한 다각형)·JSON 불러오기
- **모바일** — 도형 선택 시 하단 액션 바로 반시계/시계 회전·뒤집기, 작은 화면에서 GUI 햄버거 메뉴
